...
import pickle
import numpy as np
from sklearn.ensemble import RandomForestRegressor  # For the ML model
            ...
            try:
                ...
                if videos:
                    ...
                    if user_language == 'hi':
                        ...
                        intro_text = f"{topic} பற்றிய சில வீடிయోக்கள் இங்கே உள்ளன:"
                    else:
...
@app.route('/api_knowledge_base.html')
def knowledge_base():
    return render_template('api_knowledge_base.html')
...
# Create a random forest model to predict crop yields
def create_crop_yield_model():
    """Create a simple Random Forest model for crop yield prediction."""
    # This would typically be trained on actual data and saved
    # Here we're creating a simple model as a placeholder
    model = RandomForestRegressor(n_estimators=100, random_state=42)
    
    # Create some synthetic training data
    np.random.seed(42)
    
    # Features: crop_type (encoded), soil_type (encoded), soil_pH, nitrogen, phosphorus, 
    # potassium, temperature, humidity, rainfall, irrigation (encoded), season (encoded)
    
    # Create 500 synthetic data points with 11 features
    X_train = np.random.rand(500, 11)
    
    # Generate synthetic target values with some correlation to features
    y_train = (
        2 + 
        3 * X_train[:, 0] +  # Strong impact from crop type
        2 * X_train[:, 1] +  # Strong impact from soil type
        -1 * (X_train[:, 2] - 0.65)**2 +  # Optimal pH around 6.5
        1.5 * X_train[:, 3] +  # Positive impact from nitrogen
        1.2 * X_train[:, 4] +  # Positive impact from phosphorus
        1.0 * X_train[:, 5] +  # Positive impact from potassium
        -1.5 * (X_train[:, 6] - 0.5)**2 +  # Optimal temperature (bell curve)
        -1.0 * (X_train[:, 7] - 0.6)**2 +  # Optimal humidity (bell curve)
        -1.0 * (X_train[:, 8] - 0.7)**2 +  # Optimal rainfall (bell curve)
        1.5 * X_train[:, 9] +  # Positive impact from irrigation
        1.0 * X_train[:, 10] +  # Impact from season
        np.random.normal(0, 0.5, 500)  # Random noise
    )
    
    # Ensure all yields are positive
    y_train = np.maximum(y_train, 0.1)
...
crop_yield_model = create_crop_yield_model()
...
# Add route endpoint for crop yield page
@app.route('/api_crop_yield.html')
def api_crop_yield():
    return render_template('api_crop_yield.html')

# API endpoint for crop yield prediction
@app.route('/api/predict-crop-yield', methods=['POST'])
def predict_crop_yield():
    """API endpoint for crop yield prediction using ML model."""
    try:
        ...
        irrigation = data.get('irrigation', 'partial')
        season = data.get('season', 'kharif')
        
        # Feature scaling/normalization would typically be done here
        # For simplicity, we'll just do basic scaling
        
        # Encode categorical variables
        crop_encoded = crop_type_encoder.get(crop_type, 0) / 7.0  # Normalize to 0-1
        soil_encoded = soil_type_encoder.get(soil_type, 0) / 5.0  # Normalize to 0-1
        soil_ph_norm = (soil_ph - 4) / 6.0  # Normalize pH from 4-10 to 0-1
        nitrogen_norm = min(nitrogen / 200.0, 1.0)  # Normalize, cap at 1
        phosphorus_norm = min(phosphorus / 200.0, 1.0)  # Normalize, cap at 1
        potassium_norm = min(potassium / 200.0, 1.0)  # Normalize, cap at 1
        temperature_norm = min(temperature / 50.0, 1.0)  # Normalize, cap at 1
        humidity_norm = min(humidity / 100.0, 1.0)  # Normalize to 0-1
        rainfall_norm = min(rainfall / 3000.0, 1.0)  # Normalize, cap at 1
        irrigation_norm = irrigation_encoder.get(irrigation, 0.6)  # Already normalized
        season_norm = season_encoder.get(season, 0) / 2.0  # Normalize to 0-1
        
        # Create feature vector
        features = np.array([
            crop_encoded, soil_encoded, soil_ph_norm, nitrogen_norm, 
            phosphorus_norm, potassium_norm, temperature_norm, 
            humidity_norm, rainfall_norm, irrigation_norm, season_norm
        ]).reshape(1, -1)
        
        # Make prediction
        predicted_yield = crop_yield_model.predict(features)[0]
        
        # Scale prediction based on crop type for more realistic values
        base_yields = {
            'rice': 3.5,
            'wheat': 3.0,
            'maize': 2.7,
        ...
        # Adjusting the yield to be more realistic based on the crop type
        normalized_yield = (predicted_yield - 0.5) * 2  # Convert to -1 to 3 range
        adjusted_yield = base_yield * (0.7 + normalized_yield * 0.3)  # Adjust by +/- 30%
        adjusted_yield = max(adjusted_yield, base_yield * 0.4)  # Minimum yield is 40% of base
        
        # Generate improvement suggestions based on input values
        improvement_suggestions = []
        
        if nitrogen_norm < 0.4:
            improvement_suggestions.append("Increase nitrogen application for better vegetative growth")
        
        if phosphorus_norm < 0.3:
            improvement_suggestions.append("Increase phosphorus levels for improved root development and flowering")
        
        if potassium_norm < 0.3:
            improvement_suggestions.append("Increase potassium application for better disease resistance and drought tolerance")
        
        if soil_ph < 5.5 or soil_ph > 7.5:
            improvement_suggestions.append(f"Adjust soil pH to optimal range (currently {soil_ph})")
        
        if irrigation == 'rainfed' and rainfall_norm < 0.25:
            improvement_suggestions.append("Consider supplemental irrigation for better yield stability")
        ...
        print(f"Error in crop yield prediction: {str(e)}")
        return jsonify({
            'error': 'Failed to predict crop yield',
            'message': str(e)
        }), 500

# Add route for machinery sharing page
@app.route('/api_machinery_sharing.html')
def api_machinery_sharing():
    return render_template('api_machinery_sharing.html')

# Add route for farmer community page
@app.route('/api_farmer_community.html')
def api_farmer_community():
    return render_template('api_farmer_community.html')

# Add route for knowledge repository page
@app.route('/api_knowledge_base.html')
def knowledge_base():
    return render_template('api_knowledge_base.html')

# API for listing equipment in machinery sharing
@app.route('/api/list-equipment', methods=['POST'])
@token_required
def list_equipment(current_user):
    """API for farmers to list their equipment for sharing."""
    try:
        data = request.get_json()
        ...
        return jsonify({
            ...
            'equipment_id': 'demo-' + str(int(time.time())),
            'owner_id': current_user.id,
            'created_at': datetime.now().isoformat()
        }), 201
        
    except Exception as e:
        print(f"Error listing equipment: {str(e)}")
        return jsonify({
            'status': 'error',
            'message': f'Error listing equipment: {str(e)}'
        }), 500

# API for searching available equipment
@app.route('/api/search-equipment', methods=['GET'])
def search_equipment():
    ...
    try:
        ...
        mock_equipment = [
            {
                ...
                'type': 'tractor',
                'description': '55 HP tractor suitable for medium-sized farms',
                'location': 'Kolkata Region',
                'distance': '15km',
                'available_from': '2023-08-15',
                'available_until': '2023-08-30',
                'daily_rate': 500,
                'owner': 'Rajesh Kumar',
                'status': 'available',
                'image_url': 'https://images.unsplash.com/photo-1593521470968-b8b010a94319'
            },
            {
                'id': 'equip-002',
                'name': 'Combine Harvester',
                'type': 'harvester',
                'description': 'Wheat and rice combine harvester with 3.5m cutting width',
                'location': 'Midnapore',
                'distance': '30km',
                'available_from': '2023-09-10',
                'available_until': '2023-09-25',
                'daily_rate': 2000,
                'owner': 'Sunil Patel',
                'status': 'pending',
                'image_url': 'https://images.unsplash.com/photo-1589048607250-bae391a67357'
            },
            {
                'id': 'equip-003',
                'name': 'Power Sprayer',
                'type': 'sprayer',
                'description': '12L power sprayer with 15m hose',
                'location': 'Kolkata Region',
                'distance': '8km',
                'available_from': '2023-08-01',
                'available_until': '2023-08-20',
                'daily_rate': 200,
                'owner': 'Amit Singh',
                'status': 'available',
        ...
        # Filter results based on equipment type
        if equipment_type:
            mock_equipment = [e for e in mock_equipment if e['type'] == equipment_type.lower()]
        ...
        if location:
            mock_equipment = [e for e in mock_equipment if location.lower() in e['location'].lower()]
...
# API for requesting equipment rental
@app.route('/api/request-equipment', methods=['POST'])
@token_required
def request_equipment(current_user):
    """API for requesting equipment rental."""
    try:
        data = request.get_json()
        ...
        print(f"Error requesting equipment: {str(e)}")
        return jsonify({
            'status': 'error',
            'message': f'Error requesting equipment: {str(e)}'
        }), 500

# API for forum topics
@app.route('/api/forum-topics', methods=['GET'])
def get_forum_topics():
    """API for getting forum topics."""
    try:
        ...
        mock_topics = [
            {
                'id': 'topic-001',
                'title': 'Best practices for rice cultivation in low water conditions',
            ...
            {
                'id': 'topic-002',
                'title': 'Combating fall armyworm in maize crops',
                'category': 'pest_control',
                'created_by': 'Priya Sharma',
                'created_at': '2023-07-31T09:15:00Z',
                'replies': 18,
                'last_post': 'yesterday'
            },
            {
                'id': 'topic-003',
                'title': 'PM Kisan Scheme - September 2023 payment status',
                'category': 'government_schemes',
                'created_by': 'Anand Singh',
                'created_at': '2023-07-30T14:45:00Z',
                'replies': 42,
                'last_post': '2 days ago'
            },
            {
                'id': 'topic-004',
                'title': 'Suggestions for affordable sprayer equipment',
                'category': 'farm_equipment',
                'created_by': 'Vijay Patel',
                'created_at': '2023-07-29T11:20:00Z',
                'replies': 15,
                'last_post': '3 days ago'
            },
            {
                'id': 'topic-005',
                'title': 'Market rates for organic potatoes this season',
                'category': 'market_trends',
                'created_by': 'Meena Kumari',
                'created_at': '2023-07-28T16:10:00Z',
                'replies': 9,
        ...
        if category:
            mock_topics = [t for t in mock_topics if t['category'] == category.lower()]
        ...
        return jsonify({
            'status': 'error',
            'message': f'Error getting forum topics: {str(e)}'
        }), 500

# API for creating new forum topic
...
def create_topic(current_user):
    ...
    try:
        ...
        return jsonify({
            ...
            'topic_id': 'topic-' + str(int(time.time())),
            'user_id': current_user.id,
            'created_at': datetime.now().isoformat()
        }), 201
    ...
    topics = [
        {
            ...
            "author_id": "user123",
            "posted_date": "2023-07-28",
            "replies_count": 12,
            "views_count": 45,
            "content": "I'm planning to grow wheat this winter. Looking for advice on varieties suitable for Punjab region and best practices..."
        },
        {
            "id": "topic002",
            "title": "How to maintain tractor engines during monsoon?",
            "category": "machinery",
            "author": "Amit Patel",
            "author_id": "user456",
            ...
            "replies_count": 8,
            "views_count": 36,
            "content": "My tractor engine is facing issues due to high humidity. Need advice on maintenance during monsoon season..."
        },
        {
            "id": "topic003",
            "title": "Current market rates for tomatoes in Maharashtra",
            "category": "market-pricing",
            "author": "Sanjay Deshmukh",
            "author_id": "user789",
            "posted_date": "2023-07-17",
            "replies_count": 15,
            "views_count": 72,
            "content": "I'm harvesting tomatoes next week and want to know current market prices in Pune region. Also looking for direct buyers..."
        }
    ]
    
    # Mock filtering logic - in a real app this would filter from database
    if category != 'all':
        topics = [t for t in topics if t["category"] == category]
    
    if search:
        topics = [t for t in topics if search.lower() in t["title"].lower() or search.lower() in t["content"].lower()]
...
def get_topic_detail(topic_id):
    # Mock data - in a real app this would fetch the specific topic from database
    topic = {
        "id": topic_id,
        "title": "Best practices for wheat cultivation in North India",
        "category": "crop-growing",
        ...
        "author_id": "user123",
        "posted_date": "2023-07-28",
        "views_count": 46,  # Incremented for this view
        "content": "I'm planning to grow wheat this winter. Looking for advice on varieties suitable for Punjab region and best practices for soil preparation, sowing time, and irrigation schedule.\n\nMy farm is about 5 acres and last year I had some issues with rust disease. Any suggestions on disease-resistant varieties and preventive measures would be great.\n\nAlso, if anyone has experience with minimum tillage for wheat, please share your insights.",
        "replies": [
            {
                "id": "reply001",
...
@app.route('/api/knowledge/resources', methods=['GET'])
def get_knowledge_resources():
    ...
    resources = [
        {
            "id": "res001",
            "title": "Soil Health Management for Sustainable Agriculture",
            "type": "guide",
            "category": "soil-health",
            "author": "Agricultural Extension Office",
            "published_date": "2023-07-15",
            "url": "/static/resources/soil_health_guide.pdf"
        },
        {
            "id": "res002",
            "title": "Climate-Resilient Crop Varieties for Eastern India",
            "type": "paper",
            "category": "climate-smart",
            "author": "Dr. Rajiv Sharma, ICAR",
            "published_date": "2023-07-10",
            "url": "/static/resources/climate_resilient_crops.pdf"
        },
        {
            "id": "res003",
            ...
            "author": "Agricultural University",
            "published_date": "2023-07-05",
            "url": "https://example.com/rice-cultivation-video"
        }
    ]
...
@app.route('/api/knowledge/contribute', methods=['POST'])
def contribute_resource():
    # In a real app, this would validate the input and save to a database
    # For demo purposes, just validate presence of required fields
    required_fields = ['title', 'type', 'category', 'description', 'author']
    ...
    if 'file' in request.files:
        ...
        # In a real app, save the file to storage or cloud
